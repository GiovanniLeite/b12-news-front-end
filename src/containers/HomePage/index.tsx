...
export default function HomePage({ posts, featuredPosts, errors }: HomePageProps) {
        ...
        <section>
          <div className="mainContent">
            <div
              ...
              style={{
                backgroundImage: `url(${get(items[0], 'attributes.cover.data.attributes.url', '')})`,
              }}
            >
              ...
              <br />
              <br />
              <span>{calculatePostDateTime(items[0].attributes.date)}</span>
            </div>
            ...
            <div className="rightContent">
              {items.slice(1, 3).map((item) => (
                ...
                >
                  ...
                  <br />
                  <br />
                  <span>{calculatePostDateTime(item.attributes.date)}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                (post, index) =>
                  index > 2 && (
                    <div
                      ...
                      className={`newsCard ${post.attributes.cover.data ? '' : 'noImageNewsCard'}`}
                    >
                      ...
                      <div>
                        ...
                        <span className="feedPostDateTime">{calculatePostDateTime(post.attributes.date)}</span>
                      </div>
                    </div>
                  ),
              )}
